...
function setFlightDeclarationsLocally(query_uuid, operational_intent_detail) {
    ...
    console.log("Found " + operational_intent_detail.features.length + " operational intents to set locally.");
    ...
    for (const operational_intent_feature of operational_intent_detail.features) {
        ...
        tile38_client.set('operational_intents_in_aoi', random_operational_intent_id, operational_intent_feature['geometry'], {
...
const getNearbyOperationalIntentsProcess = async (job) => {
    ...
    try {
        ...
        const blender_response = await axios_instance.get(network_flight_declarations_url);
        ...
        console.log("Found " + operational_intent_search_results.objects.length + " nearby operational intents..");
